...
class StatisticalModeler:
    ...
    def apply_adstock_transformation(
        ...
        if not (0 < decay_rate < 1):
            raise ValueError(
                f"decay_rate must be in range (0, 1), got {decay_rate}"
            )
    ...
    def fit(self, df: pd.DataFrame, test_size: float = 0.2) -> ModelResults:
        ...
        required_cols = spend_channels + ['new_customers', 'holidays', 'competitor_promo']
        ...
        baseline_result = self._fit_baseline_model(df, active_channels, train_idx, test_idx)
        
        # Fit adstock model with grid search
        print("Fitting adstock model with grid search...")
        adstock_result = self._fit_adstock_model(df, active_channels, train_idx, test_idx)
        
        # Fit saturation model with grid search
        print("Fitting saturation model with grid search...")
        saturation_result = self._fit_saturation_model(df, active_channels, train_idx, test_idx)
        ...
        r2_diff = best_result.diagnostics.r_squared_train - best_result.diagnostics.r_squared_test
        if r2_diff > 0.2:
            warnings.warn(
    ...
    def _fit_adstock_model(
        ...
        for decay in decay_rates:
            ...
            if model.aic < best_aic:
                ...
                # Feature names
                feature_names = ['const'] + adstock_cols + ['holidays', 'competitor_promo']
                X_train_df = pd.DataFrame(X_train_sm, columns=feature_names)
                X_test_df = pd.DataFrame(X_test_sm, columns=feature_names)
    ...
    def _fit_saturation_model(
        ...
        if best_result is None:
            raise RuntimeError("Saturation model fitting failed for all parameter combinations.")
    ...
    def _fit_full_model(
        ...
        for decay in decay_rates:
            for gamma in gamma_values:
                ...
                if model.aic < best_aic:
                    ...
                    # Feature names
                    feature_names = ['const'] + full_cols + ['holidays', 'competitor_promo']
                    X_train_df = pd.DataFrame(X_train_sm, columns=feature_names)
                    X_test_df = pd.DataFrame(X_test_sm, columns=feature_names)
        ...
        if best_result is None:
            raise RuntimeError("Full model fitting failed for all parameter combinations.")
    ...
    def validate_model(
        ...
        try:
            ...
            scores = cross_val_score(
                lr, X_combined, y_combined, cv=5, scoring='r2'
            )
