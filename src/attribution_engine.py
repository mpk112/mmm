...
class AttributionEngine:
    ...
    def calculate_attribution(self, df: pd.DataFrame) -> AttributionResults:
        ...
        for channel, contribution in marginal_contributions.items():
            if total_contribution > 0:
                percentage_contributions[channel] = (contribution / total_contribution) * 100
            else:
        ...
        optimal_spend_levels = {ch: 0.0 for ch in self.spend_channels if ch in df.columns}
    ...
    def optimize_budget(
        ...
        def objective(allocation: np.ndarray) -> float:
            ...
            expected_customers = self._predict_customers(allocation, active_channels, df)
        ...
        if current_customers > 0:
            expected_lift = ((expected_customers - current_customers) / current_customers) * 100
        else:
            expected_lift = 0.0
        
        # Convergence status
        convergence_status = "Success" if result.success else f"Failed: {result.message}"
    ...
    def _predict_customers(
        ...
        for i, channel in enumerate(channels):
            ...
            if model_type == 'baseline':
                ...
                decay = transformation_params[channel]['decay']
                transformed = modeler.apply_adstock_transformation(spend, decay_rate=decay)
            elif model_type == 'saturation':
                alpha = transformation_params[channel]['alpha']
                gamma = transformation_params[channel]['gamma']
